...
server.get('/api/photo/get', apiPhotoGet)

server.get('/api/photo/fetch', apiFetchPhoto)
...
async function apiUploadPhoto(req, res) {
    ...
    if (req.file == undefined) return res.send({ error: { code: 400, description: 'no file' } })
...
// reques: hash (string)
//response: error={code=id, description=string}, response={code=int, message=[photo]}
async function apiPhotoGet(req, res){
    let user = req.query

    let result = await checkLogin(user.hash, res) // proverka loginas.
    if (result != 200) return;

    let hash = await db.base.collection('sessions').find({hash: user.hash}).toArray()
    let username = hash[0].username

    let photos = await db.base.collection('photos').find({username}).toArray()//{username} == {username: username}

    for (let photo of photos){
        delete photo._id
    }

    return res.send({response: {code:200, message: photos}})

}


// reques: hash=string, id=string
//response: 404 or photo
async function apiFetchPhoto(req, res){
    const query = req.query
    let result = await checkLogin(query.hash, res) // proverka loginas.
    if (result != 200) return;

    let hash = await db.base.collection('sessions').find({hash: query.hash}).toArray()
    let username = hash[0].username

    let photo = await db.base.collection('photos').find({username, id: query.id}).toArray()
    ...
    if (photo.length === 0){
        ...
        res.setHeader('Content-Type',photo[0].type)
        res.sendFile(__dirname + '/photos/' + query.id)
    }
